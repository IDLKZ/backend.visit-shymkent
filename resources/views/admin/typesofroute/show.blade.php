...
@section("content")
    ...
    <div class="page-content">
        ...
        <div class="row bg-white py-5">
            ...
            <div class="col-md-8">
                ...
                <div class="d-flex justify-content-around">
                    <a class="btn btn-warning" href="{{route("route_types.edit",$category->id)}}">{{__("admin.change")}}</a>
                    <form method="post" action="{{route("route_types.destroy",$category->id)}}">
                        @csrf
                        @method("delete")
        ...
        <div class="row bg-white">
            @if($category->routesTypes)
                @if($category->routesTypes->isNotEmpty())
                    <div class="table-responsive">
                        <table id="dataTableExample" class="table">
                            ...
                            <tbody>
                            @foreach($category->routesTypes as $route)
                                <tr>
                                    <td>{{$route->route->id}}</td>
                                    <td><img src="{{$route->route->getFile('image')}}" width="50"></td>
                                    <td>{{$route->route->title}}</td>
                                    <td>{{$route->route->category->title}}</td>
                                    <td>
                                        @if($route->route->types)
                                            @if($route->route->types->isNotEmpty())
                                                <ul>
                                                    @foreach($route->route->types as $type)
                                    ...
                                    <td>
                                        @if($route->route->organizatorsRoute)
                                            @if($route->route->organizatorsRoute->isNotEmpty())
                                                <ul>
                                                    @foreach($route->route->organizatorsRoute as $organizator)
                                    ...
                                    <td>{{$route->route->distance}}</td>
                                    <td>{{$route->route->time}}</td>
                                    <td>
                                        ...
                                        @elseif($route->route->status == 0)
                                            <span class="badge bg-danger text-white">
                                                            {{__("admin.not_status")}}
                                                        </span>
                                        @elseif($route->route->status == -1)
                                    ...
                                    <td class="d-flex">
                                        <div class="btn-group dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{route('routes.show', $route->route->id)}}">{{__("admin.info")}}</a>
                                                <a class="dropdown-item" href="{{route('routes.edit', $route->route->id)}}">{{__("admin.change")}}</a>
